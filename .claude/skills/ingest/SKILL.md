...
1. **장부 대조 (기계화)** — `python3 "90. Settings/Scripts/ledger.py" check "<식별키>"` (`origin_url` 또는 도서관 페이지 id — 파일이 없으면 스크립트가 헤더와 함께 생성). 장부 마크다운을 직접 편집하지 않는다:
...
5. **위키 컴파일** — `20. Wiki/`:
   ...
   - **도메인 좌표**: why_collected 답에서 `domains` 값을 확정한다 — `90. Settings/Domain_Registry.md` 등재 값만, 다중 허용, 판단 안 서면 사용자에게 질문.
   - **summary**: 모든 신규·갱신 페이지에 `summary`(120자 이내 한 줄) 필수 기입 — index·WS 맵 생성의 원천.
   - (탐색 게이트 모듈 활성 시) 새 concept·entity·guide·map은 `explored: false`로 생성, `confidence: high` 단정에는 편향 점검 콜아웃.
6. **기록 (기계화)** — `python3 "90. Settings/Scripts/build_index.py"`(index·WS 맵 파생 재생성 — 수동 편집 금지) → `python3 "90. Settings/Scripts/lint.py"`(🔴 있으면 해소 후 진행). `00. Core/log.md` append, 장부는 `ledger.py add --key … --title … --edited … --raw … --status 신규`. Inbox에서 온 항목이면 제거.
